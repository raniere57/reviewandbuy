// Finessa Review Specific JavaScript

document.addEventListener('DOMContentLoaded', function() {
    initProgressBars();
    initTimelineAnimations();
    initRatingAnimations(4.8);
    initVideoSection('Finessa Review Video');
    initConversionTracking();
    initHeaderButton();
    loadRelatedReviews('finessa');
    initMobileMenu();
});
...
function initRatingAnimations(targetRating) {
    ...
    ratingElements.forEach(rating => {
        ...
        // Number count up (verdict section)
...
function initVideoSection(label) {
    ...
    const videoObserver = new IntersectionObserver(function(entries) {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                setTimeout(() => { videoWrapper.style.transform = 'scale(1)'; videoWrapper.style.opacity = '1'; }, 200);
                setTimeout(() => { productImageLarge.style.transform = 'translateY(0)'; productImageLarge.style.opacity = '1'; }, 400);
    ...
    if (iframe) {
        ...
        videoWrapper.addEventListener('click', function() { trackReviewInteraction('video_clicked', label); });
    }
}
...
function trackAffiliateClick(buttonType, buttonText, position) {
    const trackingData = {
        ...
        product: 'Finessa',
        affiliate_link: 'https://myfinessa.com/?hopId=574b05db-369c-4a56-9185-52934241bb5a',
    ...
    exitModal.innerHTML = `
        <div class="exit-modal-backdrop">
            <div class="exit-modal-content">
                ...
                <p>Get <strong>Finessa</strong> with our <strong>180-day money-back guarantee</strong></p>
                <div class="exit-offer">
                    <span class="exit-price">$39</span>
                    <span class="exit-guarantee">Risk-Free + Selected FREE Shipping</span>
                </div>
                <a href="https://myfinessa.com/?hopId=574b05db-369c-4a56-9185-52934241bb5a" target="_blank" class="exit-cta">Claim Your Discount Now</a>
            </div>
        </div>
    `;
    const style = document.createElement('style');
    style.textContent = `
        .exit-intent-modal { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 10000; animation: fadeIn 0.3s ease-out; }
        .exit-modal-backdrop { background: rgba(0,0,0,0.8); width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; padding: 2rem; }
        .exit-modal-content { background: white; padding: 3rem; border-radius: 20px; text-align: center; max-width: 500px; position: relative; animation: scaleIn 0.3s ease-out; }
        ...
        .exit-cta { display: inline-block; background: linear-gradient(135deg, var(--product-primary), var(--product-secondary)); color: white; padding: 1rem 2rem; border-radius: 50px; text-decoration: none; font-weight: 700; transition: transform 0.3s ease; }
...
function loadRelatedReviews(currentReviewId) {
    const allReviews = [
        { id: 'javaburn', name: 'JavaBurn', rating: 4.5, description: 'Coffee metabolism booster for weight management.', url: '../reviews/javaburn.html', icon: 'fas fa-coffee' },
        { id: 'gluco6', name: 'Gluco6', rating: 4.6, description: 'Blood sugar support with plant-based ingredients.', url: '../reviews/gluco6.html', icon: 'fas fa-heartbeat' },
        { id: 'pinealguardian', name: 'Pineal Guardian', rating: 4.7, description: 'Sleep and melatonin support formula.', url: '../reviews/pinealguardian.html', icon: 'fas fa-moon' },
        { id: 'hepatoburn', name: 'HepatoBurn', rating: 4.6, description: 'Liver purification and weight support.', url: '../reviews/hepatoburn.html', icon: 'fas fa-fire' },
        { id: 'quietumplus', name: 'Quietum Plus', rating: 4.8, description: 'Ear health and tinnitus support.', url: '../reviews/quietumplus.html', icon: 'fas fa-volume-mute' },
        { id: 'audifort', name: 'Audifort', rating: 4.7, description: 'Hearing health support formula.', url: '../reviews/audifort.html', icon: 'fas fa-ear-listen' },
        { id: 'jointgenesis', name: 'Joint Genesis', rating: 4.8, description: 'Joint health and flexibility support.', url: '../reviews/jointgenesis.html', icon: 'fas fa-bone' },
        { id: 'prodentim', name: 'ProDentim', rating: 4.9, description: 'Oral probiotic for teeth and gums.', url: '../reviews/prodentim.html', icon: 'fas fa-tooth' },
        { id: 'prostavive', name: 'ProstaVive', rating: 4.7, description: 'Prostate health support.', url: '../reviews/prostavive.html', icon: 'fas fa-male' },
        { id: 'mitolyn', name: 'Mitolyn', rating: 4.5, description: 'Cellular energy support.', url: '../reviews/mitolyn.html', icon: 'fas fa-bolt' },
        { id: 'puremoringa', name: 'Pure Moringa', rating: 4.4, description: 'Superfood powder for wellness.', url: '../reviews/puremoringa.html', icon: 'fas fa-leaf' }
    ];
