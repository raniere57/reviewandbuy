// Nerve Fresh Review Specific JavaScript
...
(function initEntrypoint(){
    const init = () => {
        ...
        initVideoSection('Nerve Fresh Review Video');
        initConversionTracking();
        initHeaderButton();
        initMobileMenu();
        loadRelatedReviews('nervefresh');
    };
...
function trackAffiliateClick(buttonType, buttonText, position){
    console.log('Affiliate Click Tracked:', {buttonType,buttonText,position,product:'Nerve Fresh',affiliate_link:'https://7b5c7obhm9s3k6cclexambtev4.hop.clickbank.net',timestamp:new Date().toISOString(),page_url:window.location.href,user_agent:navigator.userAgent});
}
...
function showExitIntentOffer(){
    ...
    <div class="exit-modal-backdrop">
        <div class="exit-modal-content">
            ...
            <p>Get <strong>Nerve Fresh</strong> with our <strong>money-back guarantee</strong></p>
            <div class="exit-offer">
                <span class="exit-price">$39</span>
                <span class="exit-guarantee">Risk-Free + Selected FREE Shipping</span>
            </div>
            <a href="https://7b5c7obhm9s3k6cclexambtev4.hop.clickbank.net" target="_blank" class="exit-cta">Claim Your Discount Now</a>
        </div>
    </div>`;
...
function loadRelatedReviews(currentId){
    const all=[
        ...
        {id:'zencortex',name:'ZenCortex',rating:4.8,description:'Natural hearing health support for ear function and auditory wellness.',url:'./zencortex.html',icon:'fas fa-ear-listen'},
        {id:'titanflow',name:'TitanFlow',rating:4.8,description:'Natural male enhancement support for performance and vitality.',url:'./titanflow.html',icon:'fas fa-bolt'}
    ];
...
(function enhanceVideoErrorHandling(){
    const onReady = () => {
        ...
        const youtubeUrl = "https://www.youtube.com/watch?v=5PVB1d1_tIw";
