// SonoVive Review Specific JavaScript
...
(function initEntrypoint(){
    const init = () => {
        ...
        initVideoSection('SonoVive Sleep Support Review Video');
        initConversionTracking();
        initHeaderButton();
        initMobileMenu();
        loadRelatedReviews('sonovive');
        initVideoFallback();
    };
...
function trackAffiliateClick(buttonType, buttonText, position){
    console.log('Affiliate Click Tracked:', {buttonType,buttonText,position,product:'SonoVive',affiliate_link:'https://4843bbbgv7v0lw6j2mvlrkuq74.hop.clickbank.net',timestamp:new Date().toISOString(),page_url:window.location.href,user_agent:navigator.userAgent});
}
...
function showExitIntentOffer(){
    ...
    <div class="exit-modal-backdrop">
        <div class="exit-modal-content">
            ...
            <p>Get <strong>SonoVive</strong> with our <strong>money-back guarantee</strong></p>
            <div class="exit-offer">
                <span class="exit-price">$39</span>
                <span class="exit-guarantee">Risk-Free + Selected FREE Shipping</span>
            </div>
            <a href="https://4843bbbgv7v0lw6j2mvlrkuq74.hop.clickbank.net" target="_blank" class="exit-cta">Claim Your Discount Now</a>
        </div>
    </div>`;
...
function loadRelatedReviews(currentId){
    const all=[
        {id:'yusleep',name:'YuSleep',rating:4.7,description:'Advanced sleep support for restful nights.',url:'./yusleep.html',icon:'fas fa-moon'},
        {id:'renew',name:'Renew',rating:4.8,description:'Deep sleep and human growth hormone support.',url:'./renew.html',icon:'fas fa-spa'},
        {id:'pinealguardian',name:'Pineal Guardian',rating:4.6,description:'Pineal gland support for cognitive function.',url:'./pinealguardian.html',icon:'fas fa-brain'},
        {id:'neuroquiet',name:'Neuro Quiet',rating:4.8,description:'Sleep and cognitive support for restful nights.',url:'./neuroquiet.html',icon:'fas fa-moon'},
        ...
        {id:'ikarialeanbellyjuice',name:'Ikaria Lean Belly Juice',rating:4.9,description:'Natural weight loss drink for belly fat reduction.',url:'./ikarialeanbellyjuice.html',icon:'fas fa-fire'}
    ];
...
function initVideoFallback(){
    ...
    const youtubeUrl = "https://www.youtube.com/watch?v=32w5M6AAo-Y";

    if (videoIframe && videoFallback) {
        let loadTimeout = setTimeout(() => {
            try {
                if (!videoIframe.contentWindow || videoIframe.contentWindow.length === 0 || videoIframe.contentDocument.readyState !== 'complete') {
