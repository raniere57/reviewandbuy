// Joint Restore Gummies Review Specific JavaScript
...
(function initEntrypoint(){
    const init = () => {
        ...
        initVideoSection('Joint Restore Gummies Review Video');
        initConversionTracking();
        initHeaderButton();
        initMobileMenu();
        loadRelatedReviews('jointrestoregummies');
    };
...
function trackAffiliateClick(buttonType, buttonText, position){
    console.log('Affiliate Click Tracked:', {buttonType,buttonText,position,product:'Joint Restore Gummies',affiliate_link:'https://45885qgjob-4c-4drdwfud6u6i.hop.clickbank.net',timestamp:new Date().toISOString(),page_url:window.location.href,user_agent:navigator.userAgent});
}
...
function showExitIntentOffer(){
    ...
    <div class="exit-modal-backdrop">
        <div class="exit-modal-content">
            ...
            <p>Get <strong>Joint Restore Gummies</strong> with our <strong>money-back guarantee</strong></p>
            <div class="exit-offer">
                <span class="exit-price">$39</span>
                <span class="exit-guarantee">Risk-Free + Selected FREE Shipping</span>
            </div>
            <a href="https://45885qgjob-4c-4drdwfud6u6i.hop.clickbank.net" target="_blank" class="exit-cta">Claim Your Discount Now</a>
        </div>
    </div>`;
...
function loadRelatedReviews(currentId){
    const all=[
        ...
        {id:'nervearmor',name:'Nerve Armor',rating:4.8,description:'Nerve comfort and mobility support.',url:'./nervearmor.html',icon:'fas fa-bolt'},
        {id:'mitolyn',name:'Mitolyn',rating:4.5,description:'Cellular energy support.',url:'./mitolyn.html',icon:'fas fa-bolt'},
        {id:'glucotonic',name:'GlucoTonic',rating:4.8,description:'Blood sugar and energy balance.',url:'./glucotonic.html',icon:'fas fa-heartbeat'}
    ];
